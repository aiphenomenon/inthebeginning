...
class CosmicRunnerApp {
  ...
  async init() {
    ...
    // Populate version label from the config.js single source of truth.
    // This runs once at boot; cutting a new version only requires bumping
    // APP_VERSION in config.js, not editing HTML.
    this._applyVersion();
  ...
  /**
   * Populate the document title and title-screen subtitle from the
   * APP_VERSION / ALBUM_DISPLAY_NAME constants in config.js. Called once
   * at init so there is no hardcoded version string drift between HTML
   * and the canonical source of truth.
   */
  _applyVersion() {
    const version = (typeof APP_VERSION !== 'undefined') ? APP_VERSION : '';
    const album = (typeof ALBUM_DISPLAY_NAME !== 'undefined') ? ALBUM_DISPLAY_NAME : '';
    if (version) {
      document.title = `inthebeginning bounce — ${version}`;
    }
    ...
    if (subtitle) {
      ...
      // Use an em-dash separator to match the previous hand-written style.
  ...
  async _loadMusic() {
    ...
    // In the deploy layout, album.json lives at
    // shared/audio/metadata/v1/album.json (verified via `find` at session
    // v50). Probing any other path first produced a spurious 404 on the
    // title screen before the user had even picked a sound mode.
    ...
    // Audio base URLs (where the MP3 files live) — same ordering rationale.
    const audioBases = [
      '../../shared/audio/tracks/',
      '../shared/audio/tracks/',
      'audio/',
    ...
    for (const path of albumJsonPaths) {
      try {
        ...
        console.debug(`_loadMusic: album.json not at ${path} (${e.message})`);
      }
    }

    if (!albumData) return;

    // Pick the first audioBase whose directory responds OK to a HEAD on the
    // first track. Shared-first ordering means this usually succeeds on the
    // first try with no 404 noise.
    const firstTrack = (albumData.tracks || [])[0];
    const mp3File = firstTrack?.audio_file || firstTrack?.file;
    let audioBase = audioBases[0];
    ...
    if (mp3File) {
      for (const base of audioBases) {
        try {
          ...
          console.debug(`_loadMusic: audio base ${base} probe failed (${e.message})`);
        }
      }
    }
  ...
  async _initSoundMode() {
    switch (this.soundMode) {
      ...
      case 'hifi': {
        ...
        // Walk the candidate paths. HEAD is used to probe reachability; the
        // real validation (RIFF magic bytes, size sanity, LFS pointer
        // detection) happens inside SpessaBridge.init, which throws a
        // descriptive Error on failure. Collect errors so we can report them.
        let sf2Loaded = false;
        const sf2Errors = [];
        for (const sf2Path of sf2Paths) {
          try {
            ...
            if (!testResp.ok) {
              sf2Errors.push(`${sf2Path}: HTTP ${testResp.status}`);
              continue;
            }
            sf2Loaded = await this.player._spessaBridge.init(sf2Path);
            if (sf2Loaded) break;
          } catch (e) {
            // Surface the actual SpessaBridge error so the user can act on it.
            console.error(`HiFi: ${sf2Path} failed to load:`, e);
            sf2Errors.push(`${sf2Path}: ${e.message}`);
          }
        }

        if (!sf2Loaded) {
          const summary = sf2Errors.join(' | ') || 'no candidate paths reachable';
          console.error(
            `HiFi: SoundFont could not be loaded from any candidate path.\n` +
            `  Errors: ${summary}\n` +
            `  Falling back to Synth mode.`
          );
          // Visible banner: prefix with ⚠ so the user notices the fallback.
          // This intentionally stays in the HUD until the first real track
          // name update, so it cannot be missed.
          if (this.hudTrack) {
            this.hudTrack.textContent = '⚠ HiFi unavailable — playing Synth (see console)';
            this.hudTrack.style.color = '#ff6060';
            // Clear the red warning after 8 seconds — the Synth mode will
            // overwrite the text on the next track change anyway, but this
            // ensures the warning is visible for at least that long.
            setTimeout(() => {
              if (this.hudTrack) this.hudTrack.style.color = '';
            }, 8000);
          }
        ...
        for (const catPath of catalogPaths) {
          try {
            ...
            console.debug(`HiFi: MIDI catalog probe ${catPath} failed (${e.message})`);
          }
        }
