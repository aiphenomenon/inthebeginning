...
class GamePlayer {
  ...
  constructor(musicSync) {
    ...
    // MusicGenerator feeds both SYNTH and WASM modes (Approach C: WASM mode
    // uses compositional generation via MusicGenerator rendered through the
    // WASM audio worklet). Allow events through for both modes so the grid
    // visualizer receives note events in WASM mode.
    this.musicGenerator.onNoteEvent = (events) => {
      if (!this.onNoteEvent) return;
      const m = this.musicSync.mode;
      if (m === AUDIO_MODE.SYNTH || m === AUDIO_MODE.WASM) {
        this.onNoteEvent(events);
      }
    };
    if (this.wasmSynth) {
